...
required.add_argument("-c", "--input-checkm-tsv", help="Input checkm summary", action="store", required=True)
required.add_argument("-t", "--input-tax-tsv", help="Input GTDB-Tk slimmed results", action="store", required=True)
...
checkm_df = pd.read_csv(args.input_checkm_tsv, sep="\t", index_col=0)

# slimming down to those we want
wanted_checkm_cols = ["Completeness", "Contamination"]
checkm_df = checkm_df.loc[:, wanted_checkm_cols]

# renaming columns
checkm_df.columns = ["Est. Completeness (%)", "Est. Redundancy (%)"]
checkm_df.index.names = ["Assembly"]

# merging those two
combined_df = trans_df.merge(checkm_df, left_index=True, right_index=True)
