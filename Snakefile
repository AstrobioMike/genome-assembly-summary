...
rule all:
    input:
        str(glds_ID) + "-genome-summaries.tsv"
    shell:
...
if is_euk:
    rule run_eukcc:
        ...
        shell:
            """
            eukcc single --db {params.eukcc_db_dir} --threads {resources.cpus} --out {params.output_dir} --keep {input} > {log} 2>&1
